  ...
  return (
    <div className="relative" ref={caixa}>
      <button
        ...
        className={`flex items-center gap-1.5 rounded-lg border px-2 py-[0.28rem] text-[length:var(--fs-micro)] font-semibold uppercase tracking-[0.06em] transition-colors ${
          temIntervalo
        ...
        <div className="painel absolute right-0 top-[calc(100%+8px)] z-50 w-[276px] p-3.5 shadow-2xl">
            ...
            {PREDEFINIDOS.map((p) => {
              ...
              return (
                <button
                  ...
                  className={`rounded-md border px-2 py-[7px] text-[length:var(--fs-corpo)] font-semibold uppercase tracking-[0.08em] transition-colors ${
          ...
          <div className="flex flex-col gap-2.5">
            <label className="flex flex-col gap-1">
              <span className="text-[length:var(--fs-corpo)] text-[var(--ink-muted)]">De</span>
              <input
                type="date"
                value={de}
                max={ate || undefined}
                onChange={(e) => setDe(e.target.value)}
                className="rounded-lg border border-[var(--border-forte)] bg-[var(--surface-2)] px-2.5 py-2 text-[length:var(--fs-corpo-2)] text-[var(--ink)] outline-none focus:border-[var(--ink)]"
              />
            </label>
            <label className="flex flex-col gap-1">
              <span className="text-[length:var(--fs-corpo)] text-[var(--ink-muted)]">Até</span>
              <input
                type="date"
                value={ate}
                min={de || undefined}
          ...
          <button
            ...
            disabled={!de || !ate}
            className="mt-3 w-full rounded-lg bg-[var(--ink)] px-3 py-2 text-[length:var(--fs-corpo)] font-semibold uppercase tracking-[0.08em] text-black transition-opacity hover:opacity-85 disabled:opacity-40"
          >
            Aplicar intervalo
          </button>

          <p className="mt-2.5 text-[length:var(--fs-corpo)] leading-relaxed text-[var(--ink-muted)]">
